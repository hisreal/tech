...
$deductionTypes = $payrollService->listDeductionTypes();
$assignments = $payrollService->allStaffDeductions();
$totalAssigned = array_sum(array_map(static fn ($a) => $a['status'] === 'active' ? (float) $a['amount'] : 0, $assignments));

function prMoney3($amount) { return '₦' . number_format((float) $amount, 2); }
?>
<div class="payroll-page">
    <section class="pr-hero">
        <div class="breadcrumb-line"><a href="dashboard.php">Dashboard</a> <i class="fa-solid fa-chevron-right mx-2"></i> Payroll Management <i class="fa-solid fa-chevron-right mx-2"></i> Deductions</div>
        <span class="pr-kicker"><i class="fa-solid fa-hand-holding-hand"></i> Payroll Administration</span>
        <h3 class="mt-3 mb-2">Deductions</h3>
        <p class="text-muted mb-0">Manage reusable deduction types and assign them to individual staff members.</p>
    </section>
    ...
    <section class="row g-3 mb-4">
        <div class="col-sm-6 col-xl-3"><div class="summary-card"><span class="summary-icon"><i class="fa-solid fa-tags"></i></span><h4><?php echo count($deductionTypes); ?></h4><p class="text-muted mb-0">Deduction Types</p></div></div>
        ...
        <div class="col-sm-6 col-xl-3"><div class="summary-card"><span class="summary-icon danger"><i class="fa-solid fa-minus"></i></span><h4><?php echo prMoney3($totalAssigned); ?></h4><p class="text-muted mb-0">Total Active Deductions / Month</p></div></div>
    </section>

    <section class="table-card">
        <div class="toolbar d-flex align-items-center justify-content-between flex-wrap gap-3"><div><h4 class="mb-1">Deduction Types</h4><p class="text-muted mb-0">Reusable deduction definitions.</p></div><button type="button" class="btn pr-btn" id="addTypeBtn"><i class="fa-solid fa-plus me-2"></i>Add Deduction Type</button></div>
                ...
                <tbody>
                    <?php foreach ($deductionTypes as $type): ?>
                        <tr>
                            ...
                            <td><?php echo prMoney3($type['default_amount']); ?></td>
                            ...
                            <td>
                                <div class="action-row">
                                    ...
                                    <form method="post" action="deduction-type-delete.php" style="display:inline" onsubmit="return confirm('Delete this deduction type?');">
                    ...
                    <?php endforeach; ?>
                    <?php if (!$deductionTypes): ?><tr><td colspan="5" class="text-center text-muted fw-bold py-4">No deduction types configured yet.</td></tr><?php endif; ?>
                </tbody>
            </table>
        </div>
    </section>
    ...
    <section class="table-card">
        <div class="toolbar d-flex align-items-center justify-content-between flex-wrap gap-3"><div><h4 class="mb-1">Staff Deduction Assignments</h4><p class="text-muted mb-0">Assign a deduction type to a specific staff member.</p></div><button type="button" class="btn pr-btn" id="addAssignBtn"><i class="fa-solid fa-plus me-2"></i>Assign Deduction</button></div>
        <div class="table-scroll">
            <table class="table pr-table align-middle">
                <thead><tr><th>Staff</th><th>Staff No.</th><th>Deduction</th><th>Amount</th><th>Status</th><th>Actions</th></tr></thead>
                <tbody>
                    <?php foreach ($assignments as $a): ?>
                        <tr>
                            ...
                            <td><?php echo prMoney3($a['amount']); ?></td>
                            ...
                            <td>
                                <form method="post" action="staff-deduction-delete.php" style="display:inline" onsubmit="return confirm('Remove this deduction from the staff member?');">
                                    <input type="hidden" name="_token" value="<?php echo sms_e(sms_csrf_token()); ?>">
                    ...
                    <?php endforeach; ?>
                    <?php if (!$assignments): ?><tr><td colspan="6" class="text-center text-muted fw-bold py-4">No deductions assigned yet.</td></tr><?php endif; ?>
                </tbody>
            </table>
        </div>
    </section>
    ...
    <div class="modal fade" id="typeModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <form class="modal-content" method="post" action="deduction-type-store.php">
                <div class="modal-header" style="background:linear-gradient(135deg,#0f766e,#115e59);color:#fff;"><h5 class="modal-title text-white" id="typeModalTitle">Add Deduction Type</h5><button class="btn-close btn-close-white" type="button" data-bs-dismiss="modal"></button></div>
    ...
    <div class="modal fade" id="assignModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <form class="modal-content" method="post" action="staff-deduction-store.php">
                <div class="modal-header" style="background:linear-gradient(135deg,#0f766e,#115e59);color:#fff;"><h5 class="modal-title text-white">Assign Deduction</h5><button class="btn-close btn-close-white" type="button" data-bs-dismiss="modal"></button></div>
                <div class="modal-body">
                    <input type="hidden" name="_token" value="<?php echo sms_e(sms_csrf_token()); ?>">
                    <div class="row g-3">
                        ...
                        <div class="col-12"><label class="form-label">Deduction Type</label><select class="form-select" name="deduction_type_id" id="assignType" style="padding-left:12px;" required><option value="">Select type</option><?php foreach ($deductionTypes as $type): ?><option value="<?php echo (int) $type['id']; ?>" data-amount="<?php echo sms_e((string) $type['default_amount']); ?>"><?php echo sms_e($type['name']); ?></option><?php endforeach; ?></select></div>
...
(function(){
    ...
    document.getElementById('addTypeBtn').addEventListener('click', function(){
        document.getElementById('typeModalTitle').textContent = 'Add Deduction Type';
    ...
    document.querySelectorAll('.edit-type-btn').forEach(function(btn){
        btn.addEventListener('click', function(){
            document.getElementById('typeModalTitle').textContent = 'Edit Deduction Type';
