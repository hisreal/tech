...
if (!$record || !$ownsClass) {
    sms_flash_set('error', 'You are not allowed to edit that attendance record.');
    header('Location: attendance.php');
    exit;
}

$result = $attendanceService->updateStudentRecord($recordId, (string) ($_POST['status'] ?? ''), $_POST['notes'] ?? null, $currentUser);
